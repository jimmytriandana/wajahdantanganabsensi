...
class FaceTrainer:
    def __init__(self):
        """Initialize FaceNet model for face recognition training"""
        ...
        # CNN classifier for face recognition
        self.classifier = None
        self.label_encoder = LabelEncoder()
        
    def load_face_dataset(self, dataset_path="datasets/faces"):
        """Load face dataset and extract embeddings"""
    ...
    def build_classifier(self, input_dim, num_classes):
        """Build CNN classifier for face embeddings"""
        model = Sequential([
    ...
    def train(self, dataset_path="datasets/faces", model_save_path="models"):
        ...
        # Callbacks dengan logging lebih detail
        callbacks = [
            EarlyStopping(
                patience=15,  # Tingkatkan patience
                restore_best_weights=True,
                verbose=1,  # Tambahkan verbose untuk melihat kapan early stopping triggered
            ...
            ModelCheckpoint(
                ...
                verbose=1  # Tambahkan verbose
            ),
            ReduceLROnPlateau(
                patience=7,  # Tingkatkan patience
                factor=0.5,
                verbose=1  # Tambahkan verbose
            )
        ]
        
        # Train model dengan epoch lebih sedikit
        history = self.classifier.fit(
            X_train, y_train,
            validation_data=(X_test, y_test),
            epochs=20,  # Kurangi dari 50 ke 20
...
class HandGestureTrainer:
    def __init__(self):
        """Initialize MobileNetV2 for hand gesture recognition"""
        self.model = None
        self.label_encoder = LabelEncoder()
        self.input_shape = (224, 224, 3)
        
    def load_hand_dataset(self, dataset_path="dataset/hand"):
        """Load hand gesture dataset"""
    ...
    def build_model(self, num_classes):
        """Build MobileNetV2-based model for hand gesture recognition"""
        ...
        model = Sequential([
            base_model,
            GlobalAveragePooling2D(),  # Extract features (1280D)
            Dropout(0.5),
            Dense(512, activation='relu'),
            Dropout(0.3),
            Dense(256, activation='relu'),
            Dropout(0.2),
            Dense(num_classes, activation='softmax')  # Softmax classifier
        ])
    ...
    def train(self, dataset_path="datasets/hands", model_save_path="models"):
...
def main():
    ...
    if not os.path.exists("datasets/faces") or not os.path.exists("datasets/hands"):
    ...
    face_classes = [d for d in os.listdir("datasets/faces") 
                   if os.path.isdir(os.path.join("datasets/faces", d))]
    hand_classes = [d for d in os.listdir("datasets/hands") 
                   if os.path.isdir(os.path.join("datasets/hands", d))]
